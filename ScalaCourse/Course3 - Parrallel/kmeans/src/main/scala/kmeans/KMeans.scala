...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val pointMeans = points.par.groupBy(findClosest(_, means))
    means.par.map(mean => (mean, pointMeans.getOrElse(mean, GenSeq()))).toMap
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    ...
    val updatedPointMeans = update(classifiedPoints, means)
    if (!converged(eta)(means, updatedPointMeans)) kMeans(points, updatedPointMeans, eta)
    else updatedPointMeans
  }
}
