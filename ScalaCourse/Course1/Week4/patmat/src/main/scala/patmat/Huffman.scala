...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      chars.groupBy(c => c).map(tup => (tup._1,tup._2.length)).iterator.toList.sortBy(tup => tup._1)
    }
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = {
      trees match {
        case l :: r :: rest => {
          val newTree = makeCodeTree(l, r) :: rest
          newTree.sortBy(tree => weight(tree))
        }
        case _ => trees
      }
    }
   ...
   */
    def until(singletonFunc: List[CodeTree] => Boolean, combineFunc: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
      ...
      else until(singleton, combineFunc)(combineFunc(trees))
    }
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def walkCodeTree(remainingTree: CodeTree, bits: List[Bit]): List[Char] = remainingTree match {
        case Leaf (c, _) => if (bits.isEmpty) List(c) else  c :: decode(tree, bits)
        case Fork(left, right, _, _) if bits.head == 0 => walkCodeTree(left, bits.tail)
        case Fork(left, right, _, _) => walkCodeTree(right, bits.tail)
      }

      walkCodeTree(tree, bits)
    }
   ...
   */
    def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
      def prepend(b: Bit, entry: (Char,List[Bit])) : (Char, List[Bit]) = (entry._1, b :: entry._2)
