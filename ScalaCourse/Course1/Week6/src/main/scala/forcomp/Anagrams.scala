...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    //foldRight maintains the order, and final type
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val combined = x ++ (y.map(li => (li._1, -li._2)))
    val summed = combined.groupBy(x => x._1).mapValues(x => x.map(y => y._2).sum).toList
    summed.filter(x => x._2 != 0).sorted
  }
